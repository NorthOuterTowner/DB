...
bool datamanager::insertData(const std::string& dbName, const std::string& tableName, const std::vector<std::string>& values) {
    ...
    std::vector<std::string> fullValues(columnsInfo.size(), "NULL"); // 初始化全列值为NULL

    for (size_t i = 0; i < values.size(); ++i) {
        if (i < columnsInfo.size()) { // 防止越界
            fullValues[i] = values[i]; // 按顺序填充值（适用于全列插入或列顺序与表结构一致的部分列插入）
        }
    }

    // 验证插入数据
    if (!validateInsertData(tableInfo, columnsInfo, fullValues)) {
